...
Here, we focus on SVMs for decoding which can be easily
applied via `rkrnsRanker`.  Finally, we show preliminary results that
predict BOLD activation images from input eigensentences or other
features.  We briefly touch on future directions for this work.
...
- *supervision matrices* are organized as above but each row contains a $n$-vector representing a quantitative feature
...
* Its structure is "orderly" but not (to our knowledge) quantitative  

* As a substitute, machine learning methods define a (semi)-quantitative word/sentence space based on data

* Since such methods are based on real example data, they may uncover (partially) "real" structure in language
...
whichevents<-sample( 1:nrow(desmat) )[1:round(nrow(desmat)* 1/2 )]
...
if ( usedesignmat ) { # simple test using design matrix
  ...
  sccan<-sparseDecom2( inmatrix=list(matTrain,esentsAllEventsTrain),
    ...
    nvecs=10, cthresh=c(25,0), its=1, mycoption=1 )
  eseg<-eigSeg( mask, sccan$eig1 , applySegmentationToImages=FALSE )
...
```
The "high effect size" voxels are sent to a naive random forest model
...
pheatmap(eanatRank$confusionMatrix$table, cluster_rows = F, cluster_cols = F)
```
...
How much of the known network do we actually find? Right/left flip?
...
pdf('Figs/correlationsBetweenPredictedBetasAndRealBetas.pdf')
