...
if ( istest ) blocksCSVlist<-blocksCSVlist[1:4] # for testing
...
assembly2<-assembleBlocks( bmask, aalimg, labs, datadir, 
  imagepostfix,  dfn, afn, dmat, usedesignrow, imat, ncompcor=10,
  zscore=TRUE, spatialsmooth=2.0 )
dmat<-assembly2[[1]]
imat<-assembly2[[2]]
...
#      maxnoisepreds=10:16, polydegree=4, hrfShifts = 25, verbose=T )
    hrf<-ts( hemodynamicRF( 30, onsets=2, durations=1, rt=0.5,cc=0.1,a1=4,a2=12,b1=0.6, b2=0.6 ) )
    betas<-bold2betas( boldmatrix=imat[1:200,], 
      designmatrix=dmats[1:200,], baseshift=0, verbose=T,
      blockNumb=dmat$blockNumb[1:200], maxnoisepreds=2, hrfBasis=hrf,
      hrfShifts=6, polydegree=6, selectionthresh=0.01 )
# msk<-antsImageClone( subaal )
# msk[subaal==1]<-colMeans(betas$eventbetas[,])# /apply(betas$eventbetas[,],MARGIN=2,FUN=sd);
# antsImageWrite(msk, 'temp.nii.gz' )
...
b2bmat<-data.matrix( cbind( dmats[j,selcols], rowSums(dmats[j,-selcols]) ) )
# btsc1<-bold2betas( boldmatrix=data.matrix(imat[j,]), whichcols=1:(ncol(b2bmat)-1),
#      designmatrix=b2bmat, baseshift=0, verbose=T,
#      blockNumb=myruns, maxnoisepreds=0, hrfBasis=NA,
#      hrfShifts=25, polydegree=4, selectionthresh=0.1 )
# derivedhrf<-ts(colMeans(btsc1$runhrfs))
hrf<-hemodynamicRF( 25, onsets=2, durations=1, rt=0.5,cc=0.1,a1=4,a2=12,b1=0.9, b2=0.9 )
btsc<-bold2betas( boldmatrix=data.matrix(imat[j,]), whichcols=1:(ncol(b2bmat)-1), 
      designmatrix=b2bmat, baseshift=0, verbose=T,
      blockNumb=myruns, maxnoisepreds=2, hrfBasis=hrf,
...
mydf<-data.frame( lab=droplevels(sent[subtest]),  sl=sentlength[subtest])
mydf<-data.frame( # lab=droplevels(sent[subtest]),  
  vox=data.matrix(subbetas)[,ff]  , sl=sentlength[subtest] )
mdl<-svm( sl ~., data=mydf[inds,])
preds<-(predict( mdl, newdata=mydf[-inds,]))
print(paste("NPredVox",length(ff),"Corr",cor(preds,mydf$sl[-inds]),"N", length(preds) ))
...
subbetasvoxsel<-data.matrix(subbetas)[inds,ff]
...
mydf<-data.frame( # lab=droplevels(sent[subtest]),  
  vox=data.matrix(subbetas)[,] %*% t(ccaout), sl=sentlength[subtest])
mdl<-svm( sl ~., data=mydf[inds,])
preds<-(predict( mdl, newdata=mydf[-inds,]))
print(paste("NPredVox",length(nonzerovox),"Corr",cor(preds,mydf$sl[-inds]),"N", length(preds) ))
...
err<-sum(mydf[-inds,]$lab==preds)/nrow(mydf[-inds,])
randerr<-randomClassificationRate( mydf[-inds,]$lab, 500 )
print(paste("CCA:NPredVox",length(nonzerovox),"Correct",err*100,"N", length(preds), randerr ))
