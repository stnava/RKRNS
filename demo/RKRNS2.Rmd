...
eigsent<-eigenSentences( wiki_words, normalize=F, functiontoapply = "mean", eventdata=eventdata, eigsentbasislength=20 )
eigsent<-eigenSentences( wiki_words, normalize=T, functiontoapply = sum, eventdata=eventdata, eigsentbasislength=100 )
...
dd<-glmDenoiseR( imat[1:1000,], dmats[1:1000,], whichbase=c(4:12) , crossvalidationgroups=4, 
     baseshift = 0,  tr=0.5, hrfbasislength=20, polydegree = 12,
     maxnoisepreds=0 , selectionthresh=0.1 ,collapsedesign=T, reestimatenoisepool=F )
adafafa
  if ( ! file.exists( bfn ) ) {
    betas<-bold2betas( imat, dmats, dmat$blockNumb, 
      maxnoisepreds=2, polydegree=4 )
    antsImageWrite( as.antsImage( data.matrix( betas ) ) , bfn )
    write.csv( betas[,1:2],  bfn2, row.names=T )
  } else {
...
######################################################
j<-which( dmat$blockNumb <= 30 )
j<-dmat$blockNumb[ eventdata$eventtimes[grep('coffee',eventdata$sentences)] ]
j<-which( dmat$blockNumb %in% j )
selcols<-which( colMeans(dmats[j,]) >  0 )
# selcols<-grep('coffee', colnames(dmats) )
# j<-which( dmat$blockNumb <= 10 )
myruns<-dmat$blockNumb[j]
btsc<-bold2betas( boldmatrix=data.matrix(imat[j,]), 
      designmatrix=dmats[j,selcols], baseshift=0,
      blockNumb=myruns, maxnoisepreds=0,whichbase=4:12,
      bl=20, polydegree=12, selectionthresh=0.1 )
...
subtest<-grep('coffee',as.character(mylabs))
...
sent<-as.factor(sent)
inds<-sample(1:length(subtest),size=round(length(subtest)*9./10.))
...
mycca<-sparseDecom2( inmatrix=ccamats,
  ...
  sparseness=c( -0.001, -1 ), nvecs=10, its=8, cthresh=c(5,0),
  uselong=0, smooth=0.0, mycoption=2, inmask=c(mask,NA) )
