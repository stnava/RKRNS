...
######################################################
j<-which( dmat$blockNumb <= 24 )
selcols<-which( colMeans(dmats[j,]) >  0 )
btsc<-bold2betas( boldmatrix=data.matrix(imat[j,]), 
      designmatrix=dmats[j,selcols], baseshift=4,
      blockNumb=dmat$blockNumb[j], maxnoisepreds=10,
      bl=20, polydegree=10, selectionthresh=0.1 )
...
runfact<-as.factor(dmat$blockNumb[j][btsc$eventrows[subtest]])
sentlength<-as.numeric(dmat$nchar[j][as.numeric(btsc$eventrows)])
...
th<-0.45
ff<-which( abs(zze) > th )
mydf<-data.frame( lab=droplevels(mylabs[subtest]),  
  vox=data.matrix(subbetas)[,ff])
mydf<-data.frame( lab=sentlength,
  vox=data.matrix(subbetas)[,ff])
...
mycca<-sparseDecom2( inmatrix=ccamats,
  sparseness=c( -0.02, -0.9 ), nvecs=6, its=10, cthresh=c(5,0),
