...
labs<-as.numeric( c(  9,10,45,46,47,11,12,8,7,39,40,37,21,36 ) )
...
eigsent<-eigenSentences( wiki_words, normalize=F, functiontoapply = "nvn", eventdata=eventdata )
...
```{r eigsenttest,echo=TRUE, message=FALSE, warning=FALSE}
...
nleaveout<-2
wordoi<-'sick.child'
wordoi2<-'liked.coffee'
selector1<-grep(wordoi,eventdata$sentences)
selector2<-grep(wordoi2,eventdata$sentences)
selector<-c( selector1 , selector2 ) 
score<-0
for ( k in 1:20 ) {
  randsubset<-sample(c(  rep(TRUE,(length(selector)-nleaveout)),rep(FALSE,nleaveout)) , replace=FALSE )
  selectorTrain<-selector[ randsubset ]
  selectorTest<-selector[ c(which(!randsubset),which(!randsubset)) ]
  ...
  myclass<-templateBasedClassification( featsoi, (labels), featste, method="sccan" , mask=mask4d, eigsents=es )
#  myclass<-templateBasedClassification( featsoi, (labels), featste, method="eanat" , mask=mask4d,eigsents=classmatrix )
  print(paste(myclass$class,rownames(featste)[1]))
  pheatmap( cor( t(myclass$eanatmat), t(myclass$featuretemplate )) , cluster_rows = F, cluster_cols = F)
d<-1; ww<-( myclass$eanatmat[d,] != 0 & myclass$featuretemplate[d,] != 0  ) ; plot(myclass$eanatmat[d,ww], myclass$featuretemplate[d,ww] )
  ...
  plotANTsImage( ref, list(kk$spaceimage), slices='12x56x1' , thresh='0.1x1', color="red" , outname="sccanBasedDecoderSliceVizD.png")

  locscore<-0
  if ( sum((myclass$class[1] == rownames(featste)[1])[1]) == 1 ) locscore<-1 # else if ( sum((myclass$class[2] == rownames(featste)[1])[2]) == 1 ) locscore<-0.5
