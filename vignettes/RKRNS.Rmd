...
eigsent<-eigenSentences( wiki_words, normalize=F, functiontoapply = "nvn", eventdata=eventdata, eigsentbasislength=100 )
...
for ( k in 1:20 ) {
  ...
  es2[ inds2 , ]<-0
  es<-cbind(es1,es2)
  if ( usebrod )  myspar<-c(-0.05, -0.9 ) else  myspar<-c( -0.25, -0.9 )
  myclass<-templateBasedClassification( featsoi, (labels), featste, method="sccan" , mask=mask4d, eigsents=es, sparval = myspar )
#  myclass<-templateBasedClassification( featsoi, (labels), featste, method="eanat" , mask=mask4d,eigsents=classmatrix , sparval = myspar )
  ...
  if ( locscore == 1 ) plotANTsImage( ref, list(kk$spaceimage), slices='12x56x1' , thresh='0.1x1', color="red" , outname=vnm)
}
print(paste(k,score,score/k))
```
