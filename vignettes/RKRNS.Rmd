...
for ( k in 1:20 ) {
  ...
  es1<-sentspaceSim[ selectorTrain , ] 
  es1[ inds1 , ]<-0
  es2<-sentspaceSim[ selectorTrain , ] 
  es2[ inds2 , ]<-0
  es<-cbind(es1,es2)
...
d<-1; ww<-( myclass$eanatmat[d,] != 0 & myclass$featuretemplate[d,] != 0  ) ; plot(myclass$eanatmat[d,ww], myclass$featuretemplate[d,ww] )
  ...
  locscore<-0
  if ( sum((myclass$class == rownames(featste)[1])[1]) == 1 ) locscore<-1 # else if ( sum((myclass$class == rownames(featste)[1])[2]) == 1 ) locscore<-0.5
