...
```{r simb2b}
mypolydegree<-4
whichbase=NA    # no basis so use FIR model
whichbase=c(1:2)   # first couple bases
btsc<-bold2betas( boldmatrix=data.matrix(mat), 
      whichbase=whichbase,  # see above
      designmatrix=bb$desmat[,1:4], baseshift=2,
      blockNumb=bb$desmat$Run, maxnoisepreds=0,
      bl=12, polydegree=mypolydegree, selectionthresh=0.2 )
