...
- Reference simulation data and decoding
...
# Interpreting SCCAN prediction results 
...
used ANTs to label the cortex 

now report the labels that intersect the eanat images

## Associating classes to SCCAN predictors
Recalling: CCA maximizes $PearsonCorrelation( XW^T, ZY^T )$, 
we can study matrix $Y$ which contrasts columns of the design matrix.
```{r sccanpredictorclass}
rownames(mycca$eig2)<-levels(mylabs)
temp<-(mycca$eig2)
temp[ abs(mycca$eig2) < 0.04 ]<-0
```
...
-  Showcased some methods for studying the SCCAN results.
