...
eigsent<-eigenSentences( wiki_words, normalize=F )
...
imatf<-filterfMRI4KRNS( imat, tr=tr,  
   ...
   removeEventOverlap=NA, removeSentLengthEffects=nschar )
```

```{r makeboldfeaturespace,echo=TRUE, message=FALSE, warning=FALSE}
...
#########################################
ccafeatspace<-residuals(lm(featspace~ 1+ eventss[ eventsw > 0 ]  ))
# might pass mask4d below to get other constraints on data
source("~/code/RKRNS/R/sccanBasedDecoder.R")
ccaresults<-sccanBasedDecoder( eventdata[,], dmats, ccafeatspace[,] , sentspaceSim[,], mysparse = c( -0.01, -0.9 ), nvecs=5, cthresh=0, doEanat=F, mask=mask4d, smooth=0.0 , its=11,  interleave=T ,  locwordlist=c('red','blue') )
...
# ccaresults2<-sccanBasedDecoder( eventdata[ssubset,], dmats, residfeats , sentspaceSim[ssubset,], mysparse = c( -0.15, -0.5 ), nvecs=12, cthresh=550, doEanat=F, mask=mask4d, smooth=0.0 , its=4 )
vislist<-list()
for ( k in 1:length(ccaresults$ccaobject$eig1) ) {
  ccaimg<-ccaresults$ccaobject$eig1[[k]]
  ImageMath(4,ccaimg,"Normalize",ccaimg)
  ...
  plot(kk$timefunction,type='l'); Sys.sleep( 1.25 )
}
