...
eventoverlap<-as.numeric( eventss )
dmatsblock<-dmats
for ( i in 1:(responselength) ) {
    dmatsblock<-dmatsblock+ashift(dmats,c(i,0))
    eventoverlap<-eventoverlap+shift(eventoverlap,1)
...
eigsent<-eigenSentences( wiki_words, normalize=T )
...
imatf<-filterfMRI4KRNS( imat, tr=tr, responselength=responselength, 
   ...
   removeEventOverlap=eventoverlap, removeSentLengthEffects=NA )
imatf<-imat
```
...
#########################################
ccafeatspace<-featspace # residuals(lm(featspace~ 1 + eventoverlap[ eventsw > 0 ]  ))
# might pass mask4d below to get other constraints on data
ssubset<-( grep( "coffee", eventdata$sentences ) )
ccaresults<-sccanBasedDecoder( eventdata[ssubset,], dmats, ccafeatspace[ssubset,] , sentspaceSim[ssubset,], mysparse = c(-0.2, -0.9 ), nvecs=4, cthresh=1000, doEanat=F, mask=mask4d, smooth=0.1 , its=1 )
# compare to
