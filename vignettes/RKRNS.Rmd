...
can be found via `?bold2betas` in *RKRNS*.
```{r simb2b}
...
function in order to help set parameters.  This is available as 
output from the `bold2betas` function (and `glmDenoiseR`).  
Key parameters are the basis length for HRF estimation, 
the maximum number of noise predictors (e.g. `maxnoisepreds=4` or 
`maxnoisepreds=2:10` to search a range), the polynomial degree 
and the `selectionthresh` which should be kept below `0.5`.  
Real data processing should proceed similarly.
The latter part of the sample code extracts labels 
from the rownames of the output event beta dataframe.
This will set us up for decoding.

## Decode from $\beta$ values for each event
