...
TR=`r TR` BOLD data.  In total, we acquired `r boldhours` hours of scan time over X sessions.  Within a session, 
...
# Multivariate statistical modeling and prediction
...
_eigensentence_) at time k, $x_{lm}$ represents a point at time $l$
and space $m$ and $t$ represents the maximum temporal index within a
short window near event $Z_k$. So, if $t=0$, then the global time
...
extended with sparse methods that find rows of $W, Y$ with a
...
We decode sentences containing the words `r locwordlist`.  These
include sentences: `r wclasslevs`.  Preliminarily, we find SVD-SVM `r svmresult` and
CCA-SVM results `r ccaresult`.  The SVD-SVM uses the SVD of the matrix
$X$ as predictors.

A simple example [link](http://htmlpreview.github.io/?https://github.com/stnava/RKRNS/blob/doc/src/eigBrainLang.html)

```{r d3,results='asis',iframe = TRUE,fig.cap='A (mis)-classification network',echo=FALSE}
ww<-  misclassnetwork( nodesIn=nodedf, wclassesf[l2], pred ,outfile=NA, mycharge=-2066,zoom=F) 
```
